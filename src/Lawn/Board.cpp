...
void Board::ZombiesWon(Zombie* theZombie)
{
	...
	Reanimation* aReanim = mApp->AddReanimation(WIDESCREEN_PAD - BOARD_OFFSET, 0, MakeRenderOrder(RenderLayer::RENDER_LAYER_SCREEN_FADE, 0, 0), ReanimationType::REANIM_ZOMBIES_WON);
	aReanim->mLoopType = ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD;
...
void Board::DrawBackdrop(Graphics* g)
{
	...
	{
		g->DrawImage(Sexy::IMAGE_BACKGROUND1UNSODDED, WIDESCREEN_PAD - BOARD_OFFSET, 0);
		...
		g->DrawImage(Sexy::IMAGE_SOD1ROW, 239 - BOARD_OFFSET + WIDESCREEN_PAD, 265, aSrcRect);
	}
	else if (((mLevel == 2 || mLevel == 3) && mApp->IsFirstTimeAdventureMode()) || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED)
	{
		g->DrawImage(Sexy::IMAGE_BACKGROUND1UNSODDED, WIDESCREEN_PAD - BOARD_OFFSET, 0);
		g->DrawImage(Sexy::IMAGE_SOD1ROW, 239 - BOARD_OFFSET + WIDESCREEN_PAD, 265);
		...
		g->DrawImage(Sexy::IMAGE_SOD3ROW, 235 - BOARD_OFFSET + WIDESCREEN_PAD, 149, aSrcRect);
	}
	else if (mLevel == 4 && mApp->IsFirstTimeAdventureMode())
	{
		g->DrawImage(Sexy::IMAGE_BACKGROUND1UNSODDED, WIDESCREEN_PAD - BOARD_OFFSET, 0);
		g->DrawImage(Sexy::IMAGE_SOD3ROW, 235 - BOARD_OFFSET + WIDESCREEN_PAD, 149);
		...
		g->DrawImage(Sexy::IMAGE_BACKGROUND1, 232 - BOARD_OFFSET + WIDESCREEN_PAD, 0, aSrcRect);
	}
	else if (aBgImage)
	{
		...
		{
			g->DrawImage(aBgImage, WIDESCREEN_PAD - BOARD_OFFSET, 0);
		}
	}
	...
	if (mTutorialState == TutorialState::TUTORIAL_LEVEL_1_PLANT_PEASHOOTER)
	{
		...
		aClipG.DrawImage(Sexy::IMAGE_SOD1ROW, 239 - BOARD_OFFSET + WIDESCREEN_PAD, 265);
