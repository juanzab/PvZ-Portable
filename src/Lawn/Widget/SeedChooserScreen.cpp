...
SeedChooserScreen::SeedChooserScreen()
{
	...
	mStartButton->Resize(154 + WIDESCREEN_PAD, 545, 156, 42);
	...
	mRandomButton->Resize(332 + WIDESCREEN_PAD, 546, 100, 30);
	...
	mViewLawnButton->Resize(22 + WIDESCREEN_PAD, 561, aImageWidth, aImageHeight);
	...
	mAlmanacButton->Resize(560 + WIDESCREEN_PAD, 572, aImageWidth, aImageHeight);
	...
	mStoreButton->Resize(680 + WIDESCREEN_PAD, 572, aImageWidth, aImageHeight);
	...
	mImitaterButton->Resize(464 + WIDESCREEN_PAD, 515, Sexy::IMAGE_IMITATERSEED->mWidth, Sexy::IMAGE_IMITATERSEED->mHeight);
...
void SeedChooserScreen::Draw(Graphics* g)
{
	...
	g->DrawImage(Sexy::IMAGE_SEEDCHOOSER_BACKGROUND, WIDESCREEN_PAD, 87);
	if (mApp->HasSeedType(SEED_IMITATER))
	{
		g->DrawImage(Sexy::IMAGE_SEEDCHOOSER_IMITATERADDON, 459 + WIDESCREEN_PAD, 503);
	}
	// @Patoke: wrong local name
	TodDrawString(g, "[CHOOSE_YOUR_PLANTS]", 229 + WIDESCREEN_PAD, 110, Sexy::FONT_DWARVENTODCRAFT18YELLOW, Color::White, DS_ALIGN_CENTER);
